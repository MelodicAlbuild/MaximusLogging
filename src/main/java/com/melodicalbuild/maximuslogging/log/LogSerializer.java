package com.melodicalbuild.maximuslogging.log;

import com.melodicalbuild.maximuslogging.MaximusLogging;
import com.melodicalbuild.maximuslogging.utils.LoggedEvent;
...
public final class LogSerializer {
    private final MaximusLogging pl;
    ...
    LogSerializer(@NotNull final MaximusLogging pl, @NotNull final LoggedEvent loggedEvent, @NotNull final Event event) {
        this.pl = pl;
     ...
     */
    boolean isAnIgnoredEvent(@Nullable String condition, @Nullable String fieldName) {
    ...
    boolean serializeFields(@NotNull final Event e) {
        ...
        for (final Field f : fields) {
            ...
            try {
                ...
                // Should not happen, as it's checked on loading
            ...
            if (o instanceof String) {
            ...
            } else if (o instanceof Integer || o instanceof Boolean || o instanceof Long)
                replace("{" + fieldName + "}", String.valueOf(o));
                ...
                if (o instanceof ItemStack) item = (ItemStack) o;
                else item = ((Item) o).getItemStack();
                ...
                replace("{" + fieldName + "}", ((MerchantRecipe) o).getResult().getType().name());
    ...
    void serialize(@NotNull final Block b, @Nullable final String fieldName) {
        ...
        for (final String arg : args) {
            ...
            switch (arg) {
                case "NAME":
                    value = pl.getLogConfig().isBukkitNewApi() ? b.getBlockData().getAsString() : b.getType().name() + ":" + b.getData();
                    break;
                case "LOCWORLD":
                    value = loc.getWorld() == null ? "Unknown" : loc.getWorld().getName();
                    break;
                case "LOCX":
     ...
     * @param fieldName field name
     */
    void serialize(@NotNull final Entity e, @Nullable final String fieldName) {
        final List<String> args = loggedEvent.getConditions("ARGS");
        if (args == null) return;
     ...
     */
    void serialize(@NotNull final InventoryView iv, @NotNull final Location playerLocation) {
        final List<String> args = loggedEvent.getConditions("ARGS");
        if (args == null) return;
        ...
        for (final String arg : args) {
            ...
            switch (arg) {
                case "NAME":
                    ...
                    break;
                case "TYPE":
                    value = (iv.getType() == InventoryType.CRAFTING ? "INVENTORY" : iv.getType().name());
                    break;
                case "LOCWORLD":
                    if (loc != null && loc.getWorld() != null) value = loc.getWorld().getName();
                    break;
                case "LOCX":
                    value = loc == null ? "?" : String.valueOf(loc.getBlockX());
                    break;
                case "LOCY":
                    value = loc == null ? "?" : String.valueOf(loc.getBlockY());
                    break;
                case "LOCZ":
                    value = loc == null ? "?" : String.valueOf(loc.getBlockZ());
                    break;
                case "ITEMS":
                    final StringBuilder message = new StringBuilder();
     ...
     * @param plugin plugin
     */
    void serialize(@NotNull final Plugin plugin) {
        final List<String> args = loggedEvent.getConditions("ARGS");
        if (args == null) return;
        ...
        for (final String arg : args) {
            ...
            switch (arg) {
                case "NAME":
                    ...
                    break;
                case "DESCRIPTION":
                    value = plugin.getDescription().getDescription();
                    break;
                case "AUTHOR":
                    value = String.join(",", plugin.getDescription().getAuthors());
                    break;
                case "VERSION":
                    value = plugin.getDescription().getVersion();
                    break;
                default:
                    continue;
            }
     ...
     */
    void serialize(@NotNull final World world) {
        final List<String> args = loggedEvent.getConditions("ARGS");
     ...
     */
    void serialize(@NotNull final Chunk c) {
        final List<String> args = loggedEvent.getConditions("ARGS");
        if (args == null) return;
        for (final String arg : args) {
            final String value;
            switch (arg) {
                case "LOCWORLD":
                    value = c.getWorld().getName();
                    break;
                case "LOCX":
                    value = String.valueOf(c.getX());
                    break;
                case "LOCZ":
                    value = String.valueOf(c.getZ());
                    break;
                case "SLIME":
                    value = String.valueOf(c.isSlimeChunk());
                    break;
                default:
                    continue;
            }
